...
# ==========================================
# STEGANOGRAPHY PAYLOAD SETTINGS
# ==========================================
...
TEXT_LEN_BITS = 5
TEXT_BYTES_PER_BATCH = (PAYLOAD_BITS - TEXT_LEN_BITS) // 8  # 17 bytes

HEADER_REPETITIONS = 9


# ==========================================
# STEGANOGRAPHY BIT HELPERS
# ==========================================
...
def get_text_iterator():
    """
    Yields frames of exactly 152 bits.

    Frame format:
        000 + 3-bit index + text payload
        111 + 3-bit index + image payload
    """
    i = 0
    ...
    for start in range(0, len(text_bytes), TEXT_BYTES_PER_BATCH):
        chunk = text_bytes[start:start + TEXT_BYTES_PER_BATCH]

        payload = int_to_bits(len(chunk), TEXT_LEN_BITS)
        payload += bytes_to_bits(chunk)
        payload += "0" * (PAYLOAD_BITS - len(payload))

        yield "000" + int_to_bits(i, 3) + payload
        i = (i + 1) % 8

    # ---------- IMAGE ----------
    image_batches = encode_bmp_file_to_batches()

    for batch in image_batches:
        yield "111" + int_to_bits(i, 3) + batch
        i = (i + 1) % 8


def get_text_infinite_iterator():
    """
    Loops the payload forever so the video recorder can start late
    and still eventually capture the data.
    """
    while True:
...
def draw_stego_pixels(screen, bit_string, start_x, start_y):
    ...
    PADDING = 3
    MAX_COLS = 32
    TOTAL_SQUARES = BATCH_SIZE  # exactly 152

    padded_bit_string = bit_string[:TOTAL_SQUARES].ljust(TOTAL_SQUARES, "0")
...
def load_progress():
    """
    Return (unlocked, total_deaths, beaten).
    beaten is True once level 20 has ever been cleared.
    """
    try:
        with open(PROGRESS_FILE) as f:
            d = json.load(f)
        return (
            max(1, int(d.get("unlocked", 1))),
...
def make_screen():
    """
    Windowed SCALED display:
    fixed game resolution, vsync'd, and ready to be scaled up.
    """
    try:
...
def apply_fullscreen(want, screen):
    """
    Toggle the display to match want.
    """
    ...
    while running:
        ...
        if state == "menu":
            ...
            draw_overlay(
                screen,
                lines,
                sub="Press any key to return to the menu"
            )

        # ==========================================
        # DRAW STEGANOGRAPHY PAYLOAD
        # ==========================================
        ...
        # Calibration dot for your OpenCV scanner
        pygame.draw.rect(screen, (255, 0, 0), pygame.Rect(0, 0, 8, 8))
